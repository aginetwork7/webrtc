...
CMVideoFormatDescriptionRef CreateH265VideoFormatDescription(
  ...
  if (!reader.SeekToNextNaluOfType(H265::kVps)) {
      RTC_LOG(LS_ERROR) << "H265 Failed to SeekToNextNaluOfType";
    return nullptr;
  }
  if (!reader.ReadNalu(&param_set_ptrs[0], &param_set_sizes[0])) {
    RTC_LOG(LS_ERROR) << "H265 Failed to read VPS";
    return nullptr;
  }
  if (!reader.ReadNalu(&param_set_ptrs[1], &param_set_sizes[1])) {
    RTC_LOG(LS_ERROR) << "H265 Failed to read SPS";
    return nullptr;
  }
  if (!reader.ReadNalu(&param_set_ptrs[2], &param_set_sizes[2])) {
    RTC_LOG(LS_ERROR) << "H265 Failed to read PPS";
    return nullptr;
  }
  ...
  OSStatus status = CMVideoFormatDescriptionCreateFromHEVCParameterSets(
      ...
      &description);
  if (status != noErr) {
    RTC_LOG(LS_ERROR) << "H265 Failed to create video format description.";
    return nullptr;
  }
  return description;
}
